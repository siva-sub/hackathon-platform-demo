...
import RichTextViewer from '../components/ui/RichTextViewer'; // Added import
...
const setSchemaOrgMarkup = (schemaData: EventSchema | undefined) => {
  ...
  if (schemaData) {
    ...
    // Strip HTML from description for Schema.org
    const plainTextDescription = schemaData.description?.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() || '';
    scriptTag.textContent = JSON.stringify({...schemaData, description: plainTextDescription});
  } else {
    if (scriptTag) {
      scriptTag.remove();
    }
  ...
  useEffect(() => {
    if (hackathon?.data.status === 'approved') {
      ...
      if (hackathon.data.ogConfig) {
        const { ogTitle, ogDescription: rawOgDescription, ogImage, ogType, ogUrl } = hackathon.data.ogConfig;
        
        const effectiveOgTitle = ogTitle || hackathon.data.title;
        // Strip HTML for OG description as it's typically plain text
        const plainTextOgDescription = rawOgDescription?.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() || hackathon.data.description.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().substring(0, 160) + "...";
        ...
        setMetaTag('og:description', plainTextOgDescription);
        ...
        setMetaTag('twitter:description', plainTextOgDescription);
  ...
  return (
    <div className="space-y-8">
      ...
      >
        ...
        <RichTextViewer htmlContent={publicPageContent.heroSubtitle || description} className="text-md sm:text-lg md:text-xl mb-6 drop-shadow-md" />
        {activeStage && (
      ...
      <Card title="About This Hackathon">
        <RichTextViewer htmlContent={publicPageContent.aboutSection || "Detailed information about the hackathon, its goals, and what makes it unique will be presented here. AI can help craft compelling narratives to attract the right talent and clearly communicate the event's value proposition."} />
      </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {problemStatements.map(ps => (
              <Card key={ps.id} title={ps.title} className="bg-white dark:bg-neutral-800 shadow-md hover:shadow-lg transition-shadow">
                <RichTextViewer htmlContent={ps.description} className="text-neutral-700 dark:text-neutral-200" />
              </Card>
            ))}
          </div>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card title="Hackathon Timeline" className="bg-primary-50 dark:bg-primary-900 border-primary-200 dark:border-primary-700">
          <RichTextViewer htmlContent={timeline || "Detailed timeline will be announced soon. AI can help plan optimal durations for each phase."} />
           {stages.length > 0 && (
            <div className="mt-4">
        ...
        <Card title="Rules & Guidelines" className="bg-secondary-50 dark:bg-secondary-900 border-secondary-200 dark:border-secondary-700">
          <RichTextViewer htmlContent={rules || "Official rules and guidelines will be posted here. AI can help generate fair and comprehensive rules."} />
        </Card>
      </div>
