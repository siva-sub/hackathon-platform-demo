...
const HackathonPublicPage: React.FC = () => {
  ...
  useEffect(() => {
    if (currentHackathon) {
        const { id } = currentHackathon; 
        const { publicPageContent, title } = currentHackathon.data; 
        const customUrl = publicPageContent.imageUrl;
        const picsumUrl = `https://picsum.photos/1200/400.png?random=${id}`; 
  ...
  return (
    <div className="space-y-8">
      ...
      <Card title="About This Hackathon">
        <RichTextViewer htmlContent={publicPageContent.aboutSection || "Detailed information about the hackathon will be available here."} />
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <Card title="Timeline" className="bg-primary-50 dark:bg-primary-900 border-primary-200 dark:border-primary-700">
          <RichTextViewer htmlContent={timeline} />
        </Card>
        <Card title="Rules & Guidelines" className="bg-secondary-50 dark:bg-secondary-900 border-secondary-200 dark:border-secondary-700">
             <RichTextViewer htmlContent={rules} />
        </Card>
      </div>
