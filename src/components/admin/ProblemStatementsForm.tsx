...
const ProblemStatementsForm: React.FC = () => {
  ...
  const [newPsDescription, setNewPsDescription] = useState(''); // Will store HTML
  const [editingPs, setEditingPs] = useState<ProblemStatement | null>(null);

  const handleAddProblemStatement = () => {
    if (!newPsTitle.trim() || !newPsDescription.trim()) { // Basic check for empty HTML too
  ...
  const handleUpdateProblemStatement = () => {
    if (!editingPs || !editingPs.title.trim() || !editingPs.description.trim()) { // Basic check for empty HTML
    ...
    <Card title={`Manage Problem Statements for "${currentHackathon.data.title}"`}>
            ...
            {editingPs?.id === ps.id ? (
              <div className="space-y-3">
                ...
                <RichTextEditor
                  ...
                  onChange={(html) => setEditingPs({...editingPs!, description: html})} // Ensure editingPs is not null
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  {/* Use a viewer for rich text here if needed, or assume it's simple enough for now */}
                  <div className="text-sm text-neutral-600 dark:text-neutral-200 prose dark:prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: ps.description }} />
                </div>
      ...
      <div className="mt-6 pt-6 border-t dark:border-neutral-700">
        ...
        <div className="space-y-4">
            ...
            onChange={(html) => editingPs ? setEditingPs({...editingPs!, description: html}) : setNewPsDescription(html)}
