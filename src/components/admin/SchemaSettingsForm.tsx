...
const SchemaSettingsForm: React.FC = () => {
  ...
  useEffect(() => {
    if (currentHackathon?.data) {
      ...
      setSchemaData({
        ...
        description: existingSchema?.description || (hackathonData.description ? hackathonData.description.substring(0, 250) + "..." : ''), // Will store HTML
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => { // Removed Textarea from type
    const { name, value } = e.target;
    setSchemaData(prev => ({ ...prev, [name]: value }));
  };

  const handleDescriptionChange = (html: string) => {
    setSchemaData(prev => ({ ...prev, description: html }));
  };
    ...
    <Card title={`Schema Markup (Event) for "${currentHackathon.data.title}"`}>
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <RichTextEditor
          ...
          helperText="A short description of the event (max 250-300 chars plain text equivalent recommended). Rich text will be stored, but search engines typically process plain text from this field."
        />
