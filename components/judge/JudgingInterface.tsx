...
const JudgingInterface: React.FC<JudgingInterfaceProps> = ({ submissionId, hackathonId }) => {
  ...
  useEffect(() => {
    ...
    if (sub) {
      ...
      if (stageToJudge) {
        ...
        setGeneralComment(existingStageScoreData?.generalComment || '');
      } else {
  ...
  const handleScoreCommentChange = (criterionId: string, commentValue: string) => {
     setCurrentScores(prevScores =>
      prevScores.map(s =>
        s.criterionId === criterionId ? { ...s, comment: commentValue } : s
      )
    );
  };
  ...
  const handleSubmitDecision = async (decision: 'approve' | 'reject') => {
    ...
    try {
        scoreSubmissionAndMakeDecision(submission.id, currentStageForJudging.id, currentScores, generalComment, judgeEmail, decision); 
  ...
  const generateAiSummary = useCallback(async () => {
    ...
    const projectInfo = `
      ...
      ${submission.answers.map(a => {
        ...
        return `${question ? question.text : 'Unknown Question'}: ${a.value}`;
      }).join('\n')}
    `.trim();
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 space-y-6"> {/* This column will stack first on smaller screens */}
        ...
        <Card title={`AI Summary for ${currentStageForJudging.name} (Optional)`}>
            ...
            {aiSummary && (
                <div className="mt-2 p-3 bg-neutral-50 dark:bg-neutral-750 rounded-md prose prose-sm dark:prose-invert max-w-none whitespace-pre-line">
                   <p className="dark:text-neutral-200">{aiSummary}</p>
                </div>
            )}
        </Card>
      </div>
          ...
          {judgingMatrix.length === 0 ? (
            ...
            <form onSubmit={(e) => { e.preventDefault(); }} className="space-y-6"> {/* Removed default submit */}
                ...
                return (
                    ...
                    <p className="text-xs text-neutral-500 dark:text-neutral-200 mb-1">{criterion.description}</p>
                    <Input
                    ...
                    <Textarea
                        ...
                        placeholder="Optional: Specific comments for this criterion..."
