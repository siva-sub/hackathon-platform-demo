...
const HackathonCardWithFallbackImage: React.FC<{ hackathon: Hackathon }> = ({ hackathon }) => {
    ...
    return (
        <Card 
            ...
            className="hover:shadow-xl transition-shadow duration-300 cursor-pointer flex flex-col justify-between"
        ...
        >
          <div>
            <img 
                ...
                onError={handleError}
            />
            <p className="text-sm text-neutral-600 dark:text-neutral-300 mb-3 line-clamp-3 px-1">{hackathon.data.description}</p>
    ...
    return (
        <div className="space-y-6">
            <Card title="Available Hackathons" className="bg-gradient-to-r from-secondary-50 to-primary-50 dark:from-secondary-900 dark:to-primary-900">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-6"> {/* Adjusted grid for participant page */}
                ...
                return (
                    <div key={sub.id} className="p-4 border dark:border-neutral-700 rounded-md bg-neutral-50 dark:bg-neutral-750 shadow-sm">
                        <div className="flex flex-col sm:flex-row justify-between items-start">
                            ...
                            <div className="flex flex-col space-y-2 items-start sm:items-end flex-shrink-0 sm:ml-2 w-full sm:w-auto">
                                <Button 
...
const ParticipantPage: React.FC = () => {
  const location = useLocation();
   ...
   const anyHackathonOpenForSubmission = hackathons.some(h => h.data.status === 'approved' && h.data.isAcceptingSubmissions && h.data.problemStatements.length > 0);
        ...
        <nav className="space-y-1 sm:space-y-2">
          {navLinks.map(link => (
            ...
            >
              ...
              {link.disabled && <span className="text-xs text-red-500 dark:text-red-400 ml-auto truncate hidden sm:inline" title={link.disabledReason}>{link.disabledReason}</span>}
            </Link>
          ))}
        </nav>
      </aside>
