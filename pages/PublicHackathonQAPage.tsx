...
const PublicHackathonQAPage: React.FC = () => {
  ...
  const sortedQuestions = [...(hackathon.data.questions || [])].sort((a, b) => {
    // Sort answered questions first, then by date (newest first)
  ...
  const handleAskQuestion = () => {
    ...
    // The list will update automatically due to context changes if not using a separate API call
  };
      ...
      <Card title={`Q&A for: ${hackathon.data.title}`}>
        ...
        {currentUser?.role === 'participant' && canAskQuestion && (
          <Card title="Ask a New Question" className="mb-6 bg-primary-50 dark:bg-primary-900">
            ...
            <Button onClick={handleAskQuestion} className="mt-3" variant="primary" leftIcon={<Icons.PaperAirplane />}>
              Submit Question
            </Button>
          </Card>
        )}
          ...
          <div className="space-y-4">
            {sortedQuestions.map((q: HackathonQuestion) => (
              <Card key={q.id} className="shadow-sm">
                <div className="p-4">
                  <p className="font-semibold text-neutral-700 dark:text-neutral-100 mb-1">{q.questionText}</p>
                  ...
                  {q.answerText ? (
                    <div className="mt-3 pt-3 border-t border-neutral-200 dark:border-neutral-700">
                      ...
                      <p className="text-neutral-600 dark:text-neutral-200 whitespace-pre-line">{q.answerText}</p>
